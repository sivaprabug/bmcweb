...
inline void afterGetManagedObjectsDumpEntryCollection(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const boost::urls::url& entriesPath, const std::string& dumpType,
...
inline void afterGetManagedObjectsDumpEntryById(
    ...
    const boost::urls::url& entriesPath, const boost::system::error_code& ec,
    const dbus::utility::ManagedObjectType& objects)
{
    if (ec)
    {
...
inline void afterGetSubtreePathsDumpServiceInfo(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& dumpType, const boost::urls::url& dumpPath,
    const boost::system::error_code& ec,
